...
class FreesurferTemplate(object):
    valid_stages = (1, 2, 3)
    valid_algorithms = ('autorecon', 'subjectTemplate', 'longitudinal')

    def __init__(*args, **kwds):
        assert kwds['stage'] in valid_stages
        assert kwds['algorithm'] in valid_algorithms
        self.stage = kwds['stage']
        self.algorithm = kwds['algorithm']
        self.environment['FREESURFER_HOME'] = kwds['home']
        self.environment['SUBJECTS_DIR'] = kwds['subjDir']
        self.script = os.path.join(kwds['home'], kwds['script'])
        self.flags = kwds['flags']
        super(FreesurferTemplate, self).__init__(args, kwds)

    def run(self, subject_id, t1_fn):
        FS_SCRIPT_FN = self.script
        StageToRun = self.stage
        FS_SUBJECTS_DIR = self.environment['SUBJECTS_DIR']
        FREESURFER_HOME = self.environment['FREESURFER_HOME']
        ...
        if IsFirstNewerThanSecond(t1_fn, orig_001_mgz_fn):
            if os.path.exists(base_subj_dir):
                ...
                run_mri_convert_script(t1_fn, orig_001_mgz_fn, FS_SUBJECTS_DIR,
                                       FREESURFER_HOME, FS_SCRIPT)
        auto_recon_script = """#!/bin/bash
                export FREESURFER_HOME={FSHOME}
                export SUBJECTS_DIR={FSSUBJDIR}
                source {SOURCE_SCRIPT}
                {FSHOME}/bin/recon-all -debug -make autorecon{AUTORECONSTAGE}""".format(SOURCE_SCRIPT=FS_SCRIPT_FN,
                                                                                        FSHOME=FREESURFER_HOME,
                                                                                        AUTORECONSTAGE=StageToRun,
                                                                                        FSSUBJDIR=FS_SUBJECTS_DIR)
        if self.algorithm == "autorecon":
            auto_recon_script += " -subjid {SUBJID}".format(SUBJID=subject_id)
        elif self.algorithm == "subjectTemplate":
            auto_recon_script += " -base {SUBJID}".format(SUBJID=subject_id)
            [auto_recon_script += " -tp {timepoint}".format(timepoint=t1) for t1 in t1s]
        elif self.algorithm == 'longitudinal':
            auto_recon_script += " -long {TIMEPOINT} {SUBJID}".format(
                TIMEPOINT=t1, SUBJID=subject_id)
        else:
            raise Exception
    ...
    def scriptString(self):
        ...
        """.format(SOURCE_SCRIPT=self.script,
                   FSHOME=self.environment['FREESURFER_HOME'],
                   FSSUBJDIR=self.environment['SUBJECTS_DIR'],
                   AUTORECONSTAGE=StageToRun,
                   SUBJID=subject_id)
...
# This did not work baw_Recon1(t1_fn, wm_fn, brain_fn, FS_SUBJECTS_DIR,
# FREESURFER_HOME, FS_SCRIPT, subject_id)
    runAutoReconStage(
...
def runLongitudinalAnalysis():
    runSubjectTemplate(
        subject_id, t1s, FS_SUBJECTS_DIR, FREESURFER_HOME, FS_SCRIPT)
    runLongitudinal(
        subject_id, t1, FS_SUBJECTS_DIR, FREESURFER_HOME, FS_SCRIPT)
    dirname = os.path.join(FS_SUBJECTS_DIR, subject_id)
